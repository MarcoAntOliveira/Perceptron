...
# Não linearmente separável: todas as 3 classes
nonlinear_df = df.copy()
...
def save_kfolds(df, prefix, n_splits=5):
    ...
    for fold_num, (train_idx, test_idx) in enumerate(kf.split(X), start=1):
        ...
        train_path = f"{prefix}_fold{fold_num}_train.csv"
        test_path = f"{prefix}_fold{fold_num}_test.csv"
...
print("Conjuntos 'iris_linear' (classes 0 e 1) e 'iris_nonlinear' (todas as classes) salvos em 5 folds cada.")
