...
df_xor = pd.read_csv("data/base_xor.csv")
...
kf_xor = KFold(n_splits=4, shuffle=True, random_state=42)
...
for fold_num, (train_idx, test_idx) in enumerate(kf_xor.split(X_xor), start=1):
    ...
    train_path_xor = f"data/xor_fold{fold_num}_train.csv"
    test_path_xor = f"data/xor_fold{fold_num}_test.csv"
